...
(defn prob99 []
  (let [lines (-> "p099_base_exp.txt"
        ...
        pairs (map (fn [line]
                   ...
                   lines)
        keyfunc   (fn [[idx [base expt]]]
                    (* expt (Math/log base)))
        ]

    ;; map-indexed to get line number, inc since it is 0-index
    (inc (first (apply max-key keyfunc (map-indexed vector pairs))))))
