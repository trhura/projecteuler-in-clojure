...
(def e-fraction-values
  (let [e-fraction (fn this [k]
                     (lazy-cat [1 (* 2 k) 1]
                               (this (inc k))))]
    (e-fraction 1)))
...
(defn get-nth-term [n]
  (if (= n 1)
    2
    (let [fractions (vec (take (dec n) e-fraction-values))]
      (loop [p (/ 1 (last fractions))
             f (pop fractions)]
        (if (empty? f)
          (+ 2 p)
          (let [d (last f)]
...
(defn prob65 []
  (apply + (digit-vector (numerator (get-nth-term 100)))))
